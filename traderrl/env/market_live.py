...
class MarketLive():
    ...
    def market_order_long(self, pair="AUD_USD", unit=1000):
    ...
    def market_order_short(self, pair="AUD_USD", unit=-1000):
        client = API(access_token=self.access_token, environment='live')
        mo = MarketOrderRequest(instrument=pair, units=unit)
        r = orders.OrderCreate(self.accountID, data=mo.data)
        rv = client.request(r)
    ...
    def market_if_touched(self, pair="AUD_USD", unit=1000, p=1.08):
        client = API(access_token=self.access_token, environment='live')
        ordr = MITOrderRequest(instrument=pair, units=unit, price=p)
        r = orders.OrderCreate(self.accountID, data=ordr.data)
        rv = client.request(r)

    def position_close_long(self, pair="AUD_USD", unit=1000):
        client = API(access_token=self.access_token, environment='live')
    ...
    def candles_live(self, pair="AUD_USD", count=15, gran="M1"):
    ...
    def market_order_long_sl_tp_ts(self, sl, tp, ts, pair="AUD_USD", unit=1000):
        client = API(access_token=self.access_token, environment='live')
    ...
    def market_order_short_sl_tp_ts(self, sl, tp, ts, pair="AUD_USD", unit=-1000):
        client = API(access_token=self.access_token, environment='live')
        stopLossOnFill = StopLossDetails(price=sl)
        takeProfitOnFillOrder = TakeProfitDetails(price=tp)
        trailingStopLossOnFill = TrailingStopLossDetails(price=ts)
    ...
    def market_order_long_sl_tp(self, sl, tp, pair="AUD_USD", unit=1000):
        client = API(access_token=self.access_token, environment='live')
        stopLossOnFill = StopLossDetails(price=sl)
        takeProfitOnFillOrder = TakeProfitDetails(price=tp)
        ordr = MarketOrderRequest(
    ...
    def market_spread(self, pair="AUD_USD"):
